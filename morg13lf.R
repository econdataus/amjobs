...
read_morg <- function(aa, year, file) {
	...
	mm$emp      <- as.integer(as.integer(mm$lfsr94) <=2)
	mm$lf       <- as.integer(as.integer(mm$lfsr94) <=4)
...
nn <- aggregate(mm$weight, by=list("immig"=mm$immig,"state"=mm$statefip,"emp"=mm$emp,"lf"=mm$lf,"lesshs"=mm$lesshs,
...
#mm$lfsr94[is.na(mm$lfsr94)] <- 0
mm$stem[is.na(mm$stem)] <- 0
nn <- aggregate(mm$weight, by=list("immig"=mm$immig,"state"=as.integer(mm$statefip),"emp"=mm$emp,"lf"=mm$lf,"lesshs"=mm$lesshs,
	  "hs"=mm$hs,"somecoll"=mm$somecoll,"coll"=mm$coll,"grad"=mm$grad,"educus"=mm$educus,"stem"=mm$stem),FUN=sum, na.rm=FALSE)
	nn$year <- year
	aa <- rbind(aa, nn)
	assign('aa',aa,envir=.GlobalEnv)
...
emp_nativ_stem_coll <- aa[aa$immig==0 & aa$emp==1                & aa$stem==1 & (aa$grad==1 | aa$coll==1),]
emp_nativ_stem_grad <- aa[aa$immig==0 & aa$emp==1                & aa$stem==1 &  aa$grad==1,]
pop_nativ_stem_coll <- aa[aa$immig==0                            & aa$stem==1 & (aa$grad==1 | aa$coll==1),]
pop_nativ_stem_grad <- aa[aa$immig==0                            & aa$stem==1 &  aa$grad==1,]
...
emp_nativ_stem_coll <- aggdf(emp_nativ_stem_coll)
emp_nativ_stem_grad <- aggdf(emp_nativ_stem_grad)
pop_nativ_stem_coll <- aggdf(pop_nativ_stem_coll)
pop_nativ_stem_grad <- aggdf(pop_nativ_stem_grad)
...
colnames(emp_nativ_stem_coll)[colnames(emp_nativ_stem_coll)=="x"] <- "emp_nativ_stem_coll"
colnames(emp_nativ_stem_grad)[colnames(emp_nativ_stem_grad)=="x"] <- "emp_nativ_stem_grad"
colnames(pop_nativ_stem_coll)[colnames(pop_nativ_stem_coll)=="x"] <- "pop_nativ_stem_coll"
colnames(pop_nativ_stem_grad)[colnames(pop_nativ_stem_grad)=="x"] <- "pop_nativ_stem_grad"
...
cc <- mergedf(cc, emp_nativ_stem_coll)
cc <- mergedf(cc, emp_nativ_stem_grad)
cc <- mergedf(cc, pop_nativ_stem_coll)
cc <- mergedf(cc, pop_nativ_stem_grad)
...
#readline("Press enter to continue, escape to exit")
print("CREATE FINAL FILES morg13lf.txt and morg13lf.csv") # CHANGE FOR YEARS
write.table(cc, "morg13lf.txt") # CHANGE FOR YEARS
write.table(cc, "morg13lf.csv", sep=',') # CHANGE FOR YEARS
