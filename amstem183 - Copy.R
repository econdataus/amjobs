...
ss$imm_level <- ss$emp_imm / ss$emp_total
ss$nat_level <- ss$emp_native / ss$pop_native
print(ss)
...
title(main=paste("Figure ", nn, ": Native versus H-1B Certified Workers, ", labyears1, sep=""))
...
with(kk, plot(emp_native/1000000 ~ emp_imm_1000, cex=0.5, pch=kk$sPch, col=kk$sCol,
  ylab="Millions of Employed Native Workers", xlab=forlab1))
title(main=paste("Figure ", nn, ": Native versus H-1B Certified Workers, ", labyears2, sep=""))
with(kk, text(emp_imm_1000, emp_native/1000000, labels=year-2000, cex=0.5, pos=4))
legend("bottomright", inset=0, title="State",
c("California","Connecticut","DC","Florida","Georgia","Illinois","Maryland","Massachusetts","Michigan","New Jersey","New York","Ohio","Oregon","Pennsylvania","Texas","Virginia","Washington"),
cex=0.5, pch=gg$pch, col=gg$col, horiz=FALSE)
...
labyears <- labyears1
x11()
# Graph Native Employment Rate vs. H-1B Certification Level
nn <- nn+1
with(kk, plot(emp_pop ~ h1b_level_emp, cex=0.5, pch=kk$sPch, col=kk$sCol,
  ylab="Native Worker Employment Rate", xlab=forlab2, xlim=c(0,0.04)))
...
nn <- nn+1
with(kk, plot(emp_ln_pop ~ h1b_ln_emp, cex=0.5, pch=kk$sPch, col=kk$sCol,
...
legend("bottomleft", inset=0, cex=0.5, lty=c(2,1), col=c("red","black"), horiz=FALSE,
  c("w/o weighting","with weighting"))
lma <- (with(kk, lm(emp_ln_pop ~ h1b_ln_emp + fyear + ffips)))
abline(lma, col="red", lty=2)
lmb <- (with(kk, lm(emp_ln_pop ~ h1b_ln_emp + fyear + ffips, weights=weight)))
abline(lmb)
readline("Press enter to continue, escape to exit")

kk <- d1[d1$year >= 2003 & d1$year <= 2006,]
labyears <- "2003-2006"
x11()
# Graph Natural Logs of Native Employment Rate vs. H-1B Certification Level
nn <- nn+1
...
#legend("bottomright", inset=0, cex=0.5, lty=c(2,1), col=c("red","black"), horiz=FALSE,
#  c("w/o weighting","with weighting"))
#lma <- (with(kk, lm(emp_ln_pop ~ h1b_ln_emp + fyear + ffips, weights=weight)))
#abline(lma)
readline("Press enter to continue, escape to exit")
...
kk <- d1[d1$year >= 2002 & d1$year <= 2008,]
labyears <- "2002-2008"
x11()
# Graph Natural Logs of Native Employment Rate vs. H-1B Certification Level
nn <- nn+1
...
kk <- d1[d1$year >= 2003 & d1$year <= 2005,]
labyears <- "2003-2005"
x11()
# Graph Natural Logs of Native Employment Rate vs. H-1B Certification Level
nn <- nn+1
